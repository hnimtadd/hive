...
func newOpenRouterToolCallingClientWithConfig(model string, cfg *config.OpenAIConfig) (model.ToolCallingChatModel, error) {
	if cfg == nil {
		return nil, errors.New("openrouter configuration is empty")
	}
	openRouterConfig, err := prepareOpenRouterConfig(model, *cfg)
	if err != nil {
...
func prepareOpenRouterConfig(model string, cfg config.OpenAIConfig) (*openrouter.Config, error) {
